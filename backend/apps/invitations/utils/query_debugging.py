...
def log_slow_queries(func: Callable) -> Callable:
    ...
    def wrapper(*args, **kwargs):
        ...
        slow_queries = [q for q in counter.get_queries() if float(q["time"]) > 0.1]
...
def debug_queries(func: Callable) -> Callable:
    ...
    def wrapper(*args, **kwargs):
        ...
        logger.debug(f"{func.__name__} executed {counter.query_count} queries:")
    ...
    def _log_performance(self):
        ...
        if slow_queries:
            ...
            for i, query in enumerate(slow_queries, 1):
                logger.info(f"  {i}. {query['sql'][:150]}... ({query['time']}s)")
