...
class PermissionService:
    ...
    def can_send_invitations(self, user: 'AbstractUser') -> bool:
        ...
        return (
            self._user_has_role(user, self._roles.ADMIN) or
            self._user_has_role(user, self._roles.MANAGER) or
            self._user_has_permission(user, self._permissions.SEND_INVITATIONS)
        )
    ...
    def can_manage_all_invitations(self, user: 'AbstractUser') -> bool:
        ...
        return (
            self._user_has_role(user, self._roles.ADMIN) or
            self._user_has_permission(user, self._permissions.MANAGE_INVITATIONS)
        )
    ...
    def can_manage_invitation(self, user: 'AbstractUser', invitation_owner: 'AbstractUser') -> bool:
        """
    ...
    def can_view_stats(self, user: 'AbstractUser') -> bool:
        ...
        return (
            self._user_has_role(user, self._roles.ADMIN) or
            self._user_has_role(user, self._roles.MANAGER)
        )
    ...
    def require_manage_all_invitations_permission(self, user: 'AbstractUser') -> None:
        ...
        if not self.can_manage_all_invitations(user):
            raise PermissionError("You do not have permission to manage all invitations")
