...
class EmailTemplateService:
    ...
    def render_invitation_email(
        ...
        try:
            ...
            invitation_id = (
                getattr(invitation, "id", "unknown") if invitation else "unknown"
            )
            logger.error(
                f"Failed to render {email_type.value} email for invitation {invitation_id}: {exc}"
            )
            raise
    ...
    def send_simple_reminder(
        cls, invitation: Invitation, fail_silently: bool = False
    ) -> bool:
        """
        ...
        try:
            ...
            invitation_url = (
                f"{frontend_url}/invitations/accept?token={invitation.token}"
            )

            # Generate simple content
            subject = (
                f"⏰ Reminder: Your invitation expires in {hours_remaining} hours!"
            )
...
{invitation.invited_by.get_full_name() if invitation.invited_by else "The Team"}
            ...
            logger.info(
                f"Sent simple reminder for invitation {invitation.id} to {invitation.email}"
            )
            return True

        except Exception as exc:
            logger.error(
                f"Failed to send simple reminder for invitation {invitation.id}: {exc}"
            )
            if not fail_silently:
                raise
            return False
    ...
    def preview_email_content(
        cls, invitation: Invitation, email_type: EmailType = EmailType.INVITATION
    ) -> EmailPreview:
        """
        ...
        Raises:
            ...
            TemplateDoesNotExist: If templates are not found
        """
        return cls.render_invitation_email(invitation, email_type, preview_mode=True)
    ...
    def _validate_invitation(cls, invitation: Invitation) -> None:
        ...
        if invitation.status not in ["pending", "accepted"]:
            logger.warning(
                f"Sending email for invitation {invitation.id} with status {invitation.status}"
            )
    ...
    def _prepare_context(
        cls, invitation: Invitation, email_type: EmailType
    ) -> Dict[str, Any]:
        ...
        context = {
            ...
            "invited_by_first_name": (
                ...
                if invitation.invited_by and invitation.invited_by.first_name
                else "your inviter"
            ),
        ...
        if email_type == EmailType.REMINDER:
            ...
            context.update(
                {"is_expiry_notice": True, "expired_at": invitation.expires_at}
            )
    ...
    def _generate_subject(
        cls, invitation: Invitation, email_type: EmailType, context: Dict[str, Any]
    ) -> str:
        ...
        if email_type == EmailType.REMINDER and "hours_remaining" in context:
            ...
            if hours <= 2:
                ...
                return (
                    f"⏰ Reminder: Your {org_name} invitation expires in {hours} hours!"
                )
    ...
    def _create_email_message(
        cls, subject: str, text_content: str, html_content: str, invitation: Invitation
