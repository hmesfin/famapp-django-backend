...
class InvitationService:
    ...
    def check_email_exists(self, email: str) -> bool:
        ...
        return Invitation.objects.active().filter(
            email=email,
            status='pending'
        ).exists()
    ...
    def resend_invitation(self, invitation: Invitation, custom_message: str = None) -> dict:
        """
    ...
    def get_my_invitations(self, user: User) -> 'QuerySet':
        ...
        """
        return Invitation.objects.active().select_related(
            'invited_by',
            'accepted_by'
        ).filter(
            invited_by=user
