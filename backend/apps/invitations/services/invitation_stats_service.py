...
from apps.invitations.utils.query_debugging import (
...
class InvitationStatsService:
    ...
    def get_role_distribution(self, user: User) -> Dict[str, int]:
        ...
        # Aggregate by role
        role_stats = (
            queryset.values("role").annotate(count=Count("role")).order_by("role")
        )
    ...
    def _calculate_avg_acceptance_time(self, queryset) -> float:
        ...
        avg_seconds = queryset.filter(
            status="accepted", accepted_at__isnull=False, created_at__isnull=False
        ).aggregate(avg_time=Avg(Extract(F("accepted_at") - F("created_at"), "epoch")))[
            "avg_time"
        ]
