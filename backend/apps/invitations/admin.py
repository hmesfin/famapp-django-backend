...
class InvitationAdmin(admin.ModelAdmin):
    ...
    list_filter = ["status", "role", "is_deleted", "created_at", "expires_at"]
    ...
    fieldsets = (
        ...
        ("Sender Information", {"fields": ("invited_by",)}),
        (
            "Acceptance Information",
            {"fields": ("accepted_by", "accepted_at"), "classes": ("collapse",)},
        ),
        ("Expiry Information", {"fields": ("expires_at", "is_expired")}),
        (
            "System Fields",
            {
                "classes": ("collapse",),
    ...
    def resend_invitations(self, request, queryset):
        ...
        if resent_count:
            self.message_user(
                request, f"✅ Successfully resent {resent_count} invitation(s)."
            )
        if skipped_count:
            self.message_user(
                request,
    ...
    def cancel_invitations(self, request, queryset):
        ...
        if cancelled_count:
            self.message_user(
                request, f"✅ Successfully cancelled {cancelled_count} invitation(s)."
            )
        if skipped_count:
            self.message_user(
                request,
    ...
    def soft_delete_invitations(self, request, queryset):
        ...
        self.message_user(request, f"🗑️ Soft deleted {queryset.count()} invitation(s).")
    ...
    class Media:
        css = {"all": ("admin/css/invitation_admin.css",)}
