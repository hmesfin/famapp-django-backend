...
from apps.invitations.services.email_template_service import EmailTemplateService, EmailType
...
def send_invitation_email(self, invitation_id: int) -> bool:
    ...
    if invitation.status != 'pending':
        logger.warning(f"Invitation {invitation_id} is not pending (status: {invitation.status})")
        return False
    ...
    try:
        ...
        if success:
            logger.info(f"Successfully sent invitation email to {invitation.email} (ID: {invitation_id})")
        else:
            logger.error(f"EmailTemplateService failed to send invitation email {invitation_id}")
...
def send_invitation_reminder(self, invitation_id: int) -> bool:
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"Template reminder failed for invitation {invitation_id}: {template_exc}")
            logger.info(f"Falling back to simple reminder for invitation {invitation_id}")
        ...
        if success:
            logger.info(f"Successfully sent reminder for invitation {invitation_id} to {invitation.email}")
        else:
