...
class Command(BaseCommand):
    help = 'Analyze invitation query performance and identify optimization opportunities'
    ...
    def analyze_basic_queries(self):
        ...
        with PerformanceProfiler('optimized_invitation_list') as profiler:
            invitations = list(
                ...
                .select_related('invited_by', 'accepted_by')[:10]
            )
        
        self.report_performance('Optimized Invitation List (with select_related)', profiler)
    ...
    def report_performance(self, operation_name: str, profiler: PerformanceProfiler):
        ...
        self.stdout.write(
            style(f'{operation_name}: {query_count} queries [{status}]')
        )
        
        if self.verbose and query_count > 1:
            queries = profiler.counter.get_queries()
            for i, query in enumerate(queries, 1):
                self.stdout.write(f'  Query {i}: {query["sql"][:100]}... ({query["time"]}s)')
