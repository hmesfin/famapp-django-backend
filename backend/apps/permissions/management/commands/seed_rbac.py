...
class Command(BaseCommand):
    ...
    def _seed_permissions(self):
        ...
        for perm_data in permissions_data:
            permission, created = Permission.objects.get_or_create(
                code_name=perm_data["code_name"], defaults=perm_data
            )
            if created:
                created_count += 1
    ...
    def _seed_roles(self):
        ...
        permissions = {perm.code_name: perm for perm in Permission.objects.all()}
        ...
        roles_data = [
            ...
            },
            {
                ...
                "is_system_role": False,
                "permissions": ["view_users", "view_content", "view_reports"],
            },
            {
                "code_name": "guest",
                "name": "Guest",
        ...
        for role_data in roles_data:
            ...
            role, created = Role.objects.get_or_create(
                code_name=role_data["code_name"], defaults=role_data
            )
            ...
            if created:
                ...
                for perm_code in role_permissions:
                    if perm_code in permissions:
                        ...
                        self.stdout.write(
                            self.style.WARNING(
                                f"   ⚠️ Permission not found: {perm_code}"
                            )
                        )

                role.permissions.set(valid_permissions)
                self.stdout.write(f"     Assigned {len(valid_permissions)} permissions")
            else:
