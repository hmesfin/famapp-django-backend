...
class Permission(SimpleBaseModel):
    ...
    name = models.CharField(
        max_length=255, help_text=_("Human-readable permission name")
    )
    description = models.TextField(
        blank=True, help_text=_("Detailed description of what this permission allows")
    )
    category = models.CharField(
        max_length=50,
        blank=True,
        help_text=_(
            "Permission category for organization (e.g., 'reporting', 'admin')"
        ),
    )
    is_active = models.BooleanField(
        default=True, help_text=_("Whether this permission is currently active")
    )
...
class Role(SimpleBaseModel):
    ...
    name = models.CharField(max_length=255, help_text=_("Human-readable role name"))
    description = models.TextField(
        blank=True, help_text=_("Description of the role and its responsibilities")
    )
    ...
    is_active = models.BooleanField(
        default=True, help_text=_("Whether this role is currently active")
    )
    is_system_role = models.BooleanField(
        default=False, help_text=_("System roles cannot be deleted by users")
    )
    ...
    def get_permission_codes(self):
        ...
        return list(
            self.permissions.filter(is_active=True).values_list("code_name", flat=True)
        )
...
class UserRole(SimpleBaseModel):
    ...
    is_active = models.BooleanField(
        default=True, help_text=_("Whether this role assignment is currently active")
    )
...
class Resource(SimpleBaseModel):
    ...
    resource_id = models.CharField(
        max_length=255, help_text=_("Identifier for the specific resource")
    )
    name = models.CharField(max_length=255, help_text=_("Human-readable resource name"))
    description = models.TextField(
        blank=True, help_text=_("Description of this resource")
    )
    metadata = models.JSONField(
        default=dict, blank=True, help_text=_("Additional metadata about this resource")
    )
...
class UserPermission(SimpleBaseModel):
    ...
    permission = models.ForeignKey(
        Permission, on_delete=models.CASCADE, related_name="direct_assignments"
    )
    ...
    expires_at = models.DateTimeField(
        null=True, blank=True, help_text=_("When this permission expires (optional)")
    )
    context = models.JSONField(
        default=dict,
        blank=True,
