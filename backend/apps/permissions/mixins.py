...
class PermissionRequiredMixin(AccessMixin):
    ...
    def has_required_permissions(self, user):
        ...
        if self.require_all_permissions:
            ...
            return all(
                user.has_permission(perm) for perm in self.required_permissions
            )
        else:
    ...
    def decorator(view_func):
        ...
        def _wrapped_view(request, *args, **kwargs):
            ...
            if not has_permission:
                if request.headers.get('Accept') == 'application/json':
                    return JsonResponse({'error': 'Insufficient permissions'}, status=403)
                raise PermissionDenied("You don't have the required permissions to access this page.")
    ...
    def decorator(view_func):
        ...
        def _wrapped_view(request, *args, **kwargs):
            ...
            if not request.user.has_permission(permission_code):
                if request.headers.get('Accept') == 'application/json':
                    return JsonResponse({'error': 'Insufficient permissions'}, status=403)
                raise PermissionDenied(f"You don't have the '{permission_code}' permission.")
